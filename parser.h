...
/*
    ...
    Суффикс [_N] означает номер, 
    суффикс [_S] - строку.
*/
#define DEFAULT_N -1          // Заглушка, которая не зарезервирована терминалом. Например, путь к папке.
#define REDIRECT_INPUT_N 0    // Команда перенаправления инпута. Зарезервирована терминалом.
// ...


// #define DEFAULT_S вряд ли нам понадобится.
#define REDIRECT_INPUT_S "<<"
// ...
...
{
    int     type;           // Тип элемента, определнный дефайном выше. Например, REDIRECT_INPUT_N (0).
    char    *value;         /*
                                Строка с содержимым подстроки команды из терминала.
                                Для ключевых элементов терминала будет содержать NULL.
                            */
...
/*
    Структура для хранения основных данных о двусвязном списке.
*/
...
/*
    Дефайны - это, конечно, очень круто. Но по ним не проитерироваться.
    Сделай функцию, которая создаст массив зарезервированных строк.
    Мы его потом положим в глобальную переменную.
    Пусть он будет таким, что на позиции [<название_дефайна>_N]
    будет лежать строка [<название_дефайна>_S].

    Если соберешься маллочить массив, то маллочь и копии дефайн-строк.
    Или не маллочь ничего.

    Последний элемент массива строк - это NULL.
    Он будет служить маркером конца массива.
*/
char            **parser_get_operators(void);

/*
    Маллочит новый элемент списка и возвращает его
    или NULL в случае ошибки.
*/
t_parser_elem   *parser_get_new_elem(int type, char *value, /*...*/);
...
/*
    ...
    Не забудь сделать проверку на NULL'и и обновить все необходимые данные
